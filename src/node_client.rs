...
pub(crate) trait JobClient: Clone + Send + Sync + 'static {
    fn get_job(&self, thread_id: u32) -> Result<Job, anyhow::Error>;
...
enum StratumRequest {
    Login {
        thread_id: u32,
        reply: oneshot::Sender<Result<Job, anyhow::Error>>,
    },
    Submit {
        other_id: String,
        job_id: String,
...
pub struct NicehashStratumWorker {
    url: String,
    wallet: String,
    agent: String,
    request_rx: tokio::sync::mpsc::Receiver<StratumRequest>,
    request_tx: tokio::sync::mpsc::Sender<StratumRequest>,
}

impl NicehashStratumWorker {
    pub fn new(url: String, wallet: String, agent: String) -> Self {
    ...
    pub fn create_client(&self) -> NicehashStratumClient {
        NicehashStratumClient {
            requester: self.request_tx.clone(),
            // url: "stratum+tcp://stratum.na.mine.zpool.ca:6233".to_string(),
            // wallet: "your_wallet_address".to_string(),
            // agent: "SRBMiner-MULTI/2.8.7".to_string(),
        }
    }

    pub async fn run(mut self, mut shutdown: ShutdownSignal) -> Result<u64, anyhow::Error> {
        ...
        loop {
            tokio::select! {
                                          Some(request) = self.request_rx.recv() => {
                                              match request {
                                                  StratumRequest::Login{ thread_id, reply } => {
                ...
                for line in reader.lines() {
                           ...
                           if let Ok(job_value) = job_value {
                               ...
                               let _unused = reply.send(Ok(job)).inspect_err(|_| {
                                   error!(target: LOG_TARGET, "Failed to send job reply");
                               })
                               ;
            ...
            let _unused = reply.send(Ok(()))
                                          .inspect_err(|_| {
                                                error!(target: LOG_TARGET, "Failed to send submit reply");
                                            });
                                                  },
                                          ...
                                          _ = shutdown.wait() => {
                                              println!("Shutting down NicehashStratumWorker");
                                              break;
                                          },
                                      }
        }
...
pub(crate) struct NicehashStratumClient {
    requester: tokio::sync::mpsc::Sender<StratumRequest>,
...
impl JobClient for NicehashStratumClient {
    fn get_job(&self, thread_id: u32) -> Result<Job, anyhow::Error> {
        ...
        let res = self.requester.blocking_send(StratumRequest::Login {
            thread_id,
            reply: reply_tx,
        })?;
        // Wait for the reply
        match reply_rx.blocking_recv() {
            ...
            Err(e) => Err(anyhow!("Failed to receive job: {}", e)),
        }
    }
    ...
    fn submit(&self, other_id: String, job_id: String, nonce: u64, result: Vec<u8>) -> Result<(), anyhow::Error> {
        ...
        match reply_rx.blocking_recv() {
            ...
            Err(e) => Err(anyhow!("Failed to receive submit reply: {}", e)),
        }
    }
}
